...
export function ConnectionStatusToast({ status }: ConnectionStatusToastProps) {
  ...
  return (
    <div
      className={`pointer-events-none fixed top-3 left-1/2 z-50 -translate-x-1/2 transition-all duration-300 ${
        cfg.visible ? 'translate-y-0 opacity-100' : '-translate-y-2 opacity-0'
      }`}
    >
      <div className="flex items-center gap-2 rounded-full border border-zinc-700/50 bg-zinc-900/90 px-3.5 py-1.5 shadow-lg backdrop-blur-xl">
        <span className={`h-1.5 w-1.5 rounded-full ${cfg.dot}`} />
        <span className="text-[11px] font-medium text-zinc-300">{cfg.label}</span>
      </div>
    </div>
  )
}
